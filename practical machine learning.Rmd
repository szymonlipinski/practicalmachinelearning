...
```{r}
useTraining     <- originalTraining[,colSums(is.na(originalTraining))<nrow(originalTraining)]
useFinalTesting <- originalTesting [,colSums(is.na(originalTesting)) <nrow(originalTesting)]


trainingColNames <- colnames(useTraining)
testingColNames <- colnames(useFinalTesting)

commonColumns <- intersect(trainingColNames, testingColNames)

useFinalTesting <- useFinalTesting[commonColumns]
...
```{r}
set.seed(1)
inTrain <- createDataPartition(useTraining$classe, p=0.6, list=FALSE)
...
```{r}
predict(model, useFinalTesting, type="class")
```
